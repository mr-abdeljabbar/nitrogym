...
function App() {
  ...
  const handleWhatsAppRedirect = (e: React.FormEvent, isContactForm: boolean = false) => {
    e.preventDefault();

    let message = `Hello Nitro GYM! I want to book a free trial.\n\n`;
    ...
    if (isContactForm) {
      ...
      message += `Preferred Day: ${formData.day}\n`;
    }

    const encodedMessage = encodeURIComponent(message);
    const whatsappUrl = `https://wa.me/212700789623?text=${encodedMessage}`;
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://formspree.io/f/mbdaynez', {
        method: 'POST',
        body: data,
        headers: {
    ...
    <div className="relative bg-navy-dark min-h-screen">
        ...
        <div className="w-full flex flex-col lg:flex-row items-center px-6 lg:px-16">
          ...
          <div className={`lg:w-[45%] mb-12 lg:mb-0 transition-all duration-1000 delay-100 ${isVisible('programs') ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-16'}`}>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <button
                onClick={() => {
                  const message = encodeURIComponent("Hello Nitro GYM! I'd like to meet the team and learn more about your trainers.");
                  window.open(`https://wa.me/212700789623?text=${message}`, '_blank');
                }}
      ...
      <section id="contact" className={`animate-on-scroll bg-navy-dark py-20 lg:py-32 px-6 lg:px-16 z-50 transition-all duration-1000 ${isVisible('contact') ? 'opacity-100' : 'opacity-0'}`}>
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-16">
          ...
          <div className={`transition-all duration-1000 ${isVisible('contact') ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}>
            ...
            {contactSubmitted ? (
              <div className="glass-card rounded-2xl p-12 text-center animate-fade-in">
                <div className="w-20 h-20 bg-red-accent/20 rounded-full flex items-center justify-center mx-auto mb-6">
                  <Star className="text-red-accent fill-red-accent" size={40} />
                </div>
                <h3 className="headline-md text-white text-3xl mb-4">THANK YOU!</h3>
                <p className="text-white/70 text-lg mb-8">
                  Your request has been sent successfully. Our team will contact you shortly to confirm your session.
                </p>
                <Button
              ...
              <form className="space-y-6" onSubmit={handleContactSubmit}>
                <div className="grid sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <Input
                      ...
                      className="bg-white/5 border-white/10 text-white placeholder:text-white/30 focus:border-red-accent"
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <Select name="goal" value={formData.goal} onValueChange={(val) => handleInputChange('goal', val)}>
                  ...
                  <div>
                    ...
                    <Select name="time" value={formData.time} onValueChange={(val) => handleInputChange('time', val)}>
                      <SelectTrigger className="bg-white/5 border-white/10 text-white">
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    className="bg-white/5 border-white/10 text-white placeholder:text-white/30 focus:border-red-accent min-h-[120px]"
                    placeholder="Tell us about your fitness goals..."
                  />
                </div>
                <Button type="submit" className="w-full bg-red-accent text-white font-semibold py-6 rounded-md hover:bg-red-dark transition-all hover:-translate-y-0.5 shadow-red">
                  REQUEST BOOKING
                </Button>
              </form>
            )}
          ...
          <div className={`transition-all duration-1000 delay-200 ${isVisible('contact') ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
            <div className="rounded-2xl overflow-hidden mb-8 bg-white/5 p-8 flex items-center justify-center border border-white/10">
              <img src="/dragon-logo.png" alt="GYM NITRO Dragon Logo" className="h-64 w-auto object-contain logo-glow animate-pulse-slow" />
            </div>
            <div className="space-y-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-white font-semibold">0700-789623</p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <div className="p-3 rounded-lg bg-red-accent/10">
      ...
      <Dialog open={bookingDialogOpen} onOpenChange={setBookingDialogOpen}>
        <DialogContent className="bg-navy-light border-white/10 text-white max-w-md">
          ...
          <form className="space-y-4 mt-4" onSubmit={(e) => handleWhatsAppRedirect(e)}>
            <div>
              <label className="block font-mono text-xs uppercase tracking-wider text-white/50 mb-2">Name</label>
              <Input
                required
            ...
            <div>
              ...
              <Select value={formData.day} onValueChange={(val) => handleInputChange('day', val)}>
                <SelectTrigger className="bg-white/5 border-white/10 text-white">
